...
const CategoryTitle = styled.h5<{ active?: boolean }>`
  margin-left: 5px;
...
export const CategoryFilter = observer(() => {
  ...
  /** Fn */
  const _handleCategoryChange = (item: string) => {
  ...
  const _renderItem = (item: string, index: number) => {
    ...
    return (
      <CategoryItem key={`category_${index}`} onClick={() => _handleCategoryChange(item)}>
        <CategoryIcon>
    ...
    return (
      <FiltersWrapepr>
        ...
        <DropdownMenu active={opened}>
          {[...(category ? ['all'] : []), ...categories].filter((i) => i !== category).map(_renderItem)}
        </DropdownMenu>
      </FiltersWrapepr>
    );

  return <FiltersWrapepr>{['all', ...categories].map(_renderItem)}</FiltersWrapepr>;
});
